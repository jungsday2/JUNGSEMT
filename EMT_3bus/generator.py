...
class Generator(BaseComponent):
    ...
    def update_branch(self, V_nodes):
        ...
        i_d = (
            self.R_a    * (-v_d - self.psi_q_pp)
            + self.X_q_pp * (self.psi_d_pp - v_q)
        ) / Det
        i_q = (
            self.R_a    * (self.psi_d_pp - v_q)
            + self.X_d_pp * (v_d + self.psi_q_pp)
        ) / Det
    ...
    def stamp_Y_phasor(self, Y_matrix, omega):
        """단자 모선 대각에 Y_gen = 1/(R_a + jωL''_d) 누적."""
        Y_gen = 1.0 / (self.R_a + 1j * omega * self.L_d_pp)
        Y_matrix[self._slot, self._slot] += Y_gen

    def stamp_I_phasor(self, I_vector, omega):
        """e''_phasor 기반 Norton 전류원을 I_phasor에 누적.

        호출 시점에 회전자 상태(ψ''_d, ψ''_q, δ)가 0이면 e''=0 → 기여 없음.
        시뮬레이터가 반복 phasor solve를 하면 두번째 이후 호출에서 능동 기여.
        """
    ...
    def initialize_states(self, V_phasor, omega, dt):
        ...
        절차:
            1. 운전점 (P_op = T_m, Q_op) 지정
            2. I_gen, δ_ss 산출 (E_q_fic 사용, X_q 기준)
            3. V_dq, I_dq 변환
            4. ψ''_d, ψ''_q를 STATOR ALGEBRAIC에서 직접 backsolve (V_dq, I_dq에 자동 일치)
            5. E'_q, E'_d, E_fd를 회전자 ODE 정상상태에서 backsolve (시뮬 시 일관성 유지)

        주의:
            사용자 입력 E_fd는 Stage 6에서 운전점 일관성을 위해 OVERRIDE됨.
            (Simple GENROU의 ψ''_d ODE가 X'_d-X_l을 쓰는데, 물리 일관성은 X_d-X''_d
             를 요구하므로 X''_d=X_l이 아닌 한 사용자 E_fd값을 그대로 쓰면 stator
             algebraic이 만족되지 않음. 이를 자동 보정.)
        """
